...
public class BlazorRendererPropsGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(
            classNodeProvider
                .Where(classNode => classNode.classDeclaration.Members
                    .OfType<PropertyDeclarationSyntax>()
                    .Any(prop =>
                        ...
                        prop.AttributeLists
                            .SelectMany(a => a.Attributes)
                            .Any(attr =>
                                attr.Name.ToString() == "Parameter" ||
                                attr.Name.ToString().EndsWith(".Parameter")
                            )
                    )
                )
                .Collect(),
            (spc, classNodes) =>
            {
                ...
                foreach (var (classDeclaration, classNamespace) in classNodes)
                {
                    var className = classDeclaration.Identifier.Text;
                    codeBuilder.AppendLine($"global using {className} = {classNamespace}.{className};");
                }
