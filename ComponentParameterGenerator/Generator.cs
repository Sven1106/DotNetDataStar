...
namespace ComponentParameterGenerator;
...
public class Generator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(classNodeProvider, (spc, classNodes) =>
        {
            ...
            {
                codeBuilder.AppendLine($$"""
                                                 public static BlazorRenderer.ComponentParameters<{{className}}> CreateComponentParameters() => new(
            ...
            {
                codeBuilder.AppendLine($$"""
                                                 public static BlazorRenderer.ComponentParameters<{{className}}> CreateComponentParameters({{string.Join(", ", properties.Select(x => $"{x.Type.ToString()} {char.ToLowerInvariant(x.Identifier.Text[0]) + x.Identifier.Text.Substring(1)}"))}}) => new(
                                                     new Dictionary<string, object?>
                                                     {
                                                         {{string.Join(",\n                ", properties.Select(x => $"[nameof({x.Identifier.Text})] = {char.ToLowerInvariant(x.Identifier.Text[0]) + x.Identifier.Text.Substring(1)}"))}}
                                                     }
                                                 );
                                         """);
            }
