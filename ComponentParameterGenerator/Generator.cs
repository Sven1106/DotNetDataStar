...
public class Generator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(classNodeProvider, (spc, classNodes) =>
        {
            ...
            {
                codeBuilder.AppendLine($"""
                                                public static ComponentParameters<{className}> CreateComponentParameters() => new(
            ...
            try
            {
                ...
                string templateText = """
                                      ...
                                      namespace {{classNamespace}}
                                      {
                                          public partial class {{className}}
                                          {
                                              {{#if (eq properties.length 0)}}
                                              public static ComponentParameters<{{className}}> CreateComponentParameters() => new(
                                                  new Dictionary<string, object?>(StringComparer.OrdinalIgnoreCase)
                                              );
                                              {{else}}
                                              public static ComponentParameters<{{className}}> CreateComponentParameters(
                                                  {{join properties ", " (propSignature)}}
                                              ) => new(
                                                  new Dictionary<string, object?>
                                                  {
                                                      {{join properties ",\n                " (propEntry)}}
